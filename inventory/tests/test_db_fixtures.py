...
# Testing the data inputed into the database by the data fixtures
...
# testing the data inputed into the database by the factories
@pytest.mark.parametrize(
    "name,slug,is_active",
    [
        ("fashion", "fashion", 1),
...
def test_inventory_db_product_uniqueness_integrity(
    db, product_factory
):
...
def test_inventory_db_producttype_insert_data(
...
def test_inventory_db_stock_dataset(
    ...
    result_last_checked = result.last_checked.strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    assert result.product_inventory.id == product_inventory
...
def test_inventory_db_stock_insert_data(db, stock_factory):
    new_stock = stock_factory.create(
        product_inventory__sku="123456789"
    )
...
def test_inventory_db_insert_inventory_product_values(
    ...
    new_inv_attribute = product_with_attribute_values_factory(
        sku="123456789"
    )
